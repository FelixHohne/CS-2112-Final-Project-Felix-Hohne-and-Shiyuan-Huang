...
As a demonstration, we run two GUIs at the same time, with different permissions. The top GUI has administrator access and can make modifications to the world, whereas the bottom GUI has read only access to the world and therefore has the play and pause buttons are greyed out. The two GUIs can display the same underlying simulation state, except perhaps with possible minor timing differences due to the slightly staggered time of HTTP packets: 
