...
As a demonstration, we run two GUIs at the same time, with different permissions. The top GUI has administrator access and can make modifications to the world, whereas the bottom GUI has read only access to the world and therefore has the play and pause buttons are greyed out. Despite having different zoom conditions and different permissions, the two GUIs display the same world state, except with possible minor differences due to the slightly different arrival time of HTTP packets: 
